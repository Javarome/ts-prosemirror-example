...
/**
 * Convenience function for adding list-related node types to a map specifying the
 * @param nodes for a schema.
 * Adds:
 * - [`orderedList`](#schema-list.orderedList) as `"ordered_list"`,
 * - [`bulletList`](#schema-list.bulletList) as `"bullet_list"`, and
 * - [`listItem`](#schema-list.listItem) as `"list_item"`.
 *
 * @param `itemContent` determines the content expression for the list items.
 * If you want the commands defined in this module to apply to your list structure,
...
/**
 * Returns a command function that wraps the selection in a list with the given type an attributes.
 *
 * If `dispatch` is null, only return a value to indicate whether this is possible, but don't actually perform the change.
 */
export function wrapInList(listType: NodeType, attrs?: Object) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => any): boolean {
...
function doWrapInList(tr: Transaction, range: NodeRange, wrappers: any[], joinBefore: boolean, listType) {
  ...
  let found = 0
  for (let i = 0; i < wrappers.length; i++) if (wrappers[i].type == listType) {
    found = i + 1
  }
...
export function splitListItem(itemType: NodeType) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => any): boolean {
    ...
    if ($from.parent.content.size == 0 && $from.node(-1).childCount == $from.indexAfter(-1)) {
      // In an empty block. If this is a nested list, the wrapping list item should be split. Otherwise, bail out and
      // let next command handle lifting.
...
function liftToOuterList(state: EditorState, dispatch: (tr: Transaction) => any, itemType: NodeType, range: NodeRange) {
...
function liftOutOfList(state: EditorState, dispatch: (tr: Transaction) => any, range: NodeRange) {
  let tr = state.tr
